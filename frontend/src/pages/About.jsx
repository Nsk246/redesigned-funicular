...
function STag({ s }) {
  ...
  return (
    <span style={{ display:'inline-flex', alignItems:'center', padding:'3px 10px', borderRadius:20, fontSize:'clamp(10px,0.85vw,12px)', fontWeight:700, fontFamily:'DM Sans,sans-serif', background:c.bg, color:c.color, border:`1px solid ${c.border}`, whiteSpace:'nowrap' }}>
      S{s} · {STATE_LABELS[s]}
    </span>
  )
}
  ...
  return (
    <section style={{ display:'flex', flexDirection:'column', gap:'clamp(14px,1.4vw,20px)' }}>
      <div style={{ borderLeft:`3px solid ${accent}`, paddingLeft:'clamp(10px,1vw,14px)' }}>
        ...
        {sub && <p style={{ color:'#4a6080', fontSize:'clamp(11px,0.95vw,13px)' }}>{sub}</p>}
      </div>
      {children}
    </section>
  )
      ...
      {data.actions.map(action => (
        <div key={action.id} style={{ background:'#081020', border:'1px solid #1e3050', borderRadius:10, padding:'clamp(12px,1.2vw,16px)' }}>
          <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:12 }}>
            <span style={{ background:'#0f1f3d', border:'1px solid #1e3050', color:'#8099b8', fontSize:'clamp(10px,0.85vw,12px)', fontWeight:700, padding:'2px 8px', borderRadius:5, fontFamily:'JetBrains Mono,monospace' }}>{action.id}</span>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
            {action.probs.map((p, i) => p > 0 && (
              <div key={i} style={{ display:'flex', alignItems:'center', gap:8 }}>
                ...
                <span style={{ color:'#4a6080', fontSize:'clamp(10px,0.85vw,11px)', fontFamily:'JetBrains Mono,monospace', width:26 }}>{(p*100).toFixed(0)}%</span>
                <span style={{ display:'inline-flex', alignItems:'center', padding:'3px 10px', borderRadius:20, fontSize:'clamp(10px,0.85vw,12px)', fontWeight:700, background:sc[i].bg, color:sc[i].color, border:`1px solid ${sc[i].border}` }}>
    ...
    <div style={{ maxWidth:960, margin:'0 auto', display:'flex', flexDirection:'column', gap:'clamp(40px,5vw,64px)', paddingBottom:80 }}>
      ...
      <div style={{ textAlign:'center', padding:'clamp(32px,4vw,56px) 0 clamp(16px,2vw,24px)' }}>
        <div style={{ display:'inline-flex', alignItems:'center', background:'rgba(37,99,235,0.15)', border:'1px solid rgba(37,99,235,0.2)', borderRadius:20, padding:'4px 14px', marginBottom:16 }}>
          <span style={{ color:'#60a5fa', fontSize:'clamp(10px,0.85vw,12px)', fontWeight:600, letterSpacing:'0.06em', textTransform:'uppercase' }}>AI Agent Course Project</span>
        </div>
        ...
        <div style={{ display:'flex', justifyContent:'center', flexWrap:'wrap', gap:8 }}>
          {['RL · DQN + Q-Learning','LLM · Claude API','Multi-Agent System','FastAPI + React'].map(t => (
            <span key={t} style={{ background:'#0f1f3d', border:'1px solid #1e3050', color:'#4a6080', fontSize:'clamp(11px,0.95vw,13px)', padding:'5px 14px', borderRadius:20 }}>{t}</span>
          ))}
        </div>
      </div>
      ...
      <Section title="How Vitals Map to States" sub="The exact scoring algorithm that converts patient measurements into S0–S4" accent="#60a5fa">
        ...
        <div className="card-glow">
            ...
            {[
              ...
              return (
                <div key={f.key} style={{ background:'#081020', border:`1px solid ${scoreColor}33`, borderRadius:8, padding:'clamp(8px,0.9vw,12px)' }}>
                  <div style={{ color:'#4a6080', fontSize:'clamp(9px,0.75vw,10px)', fontWeight:600, textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:4 }}>{f.label}</div>
                  ...
                  <div style={{ color:'#4a6080', fontSize:'clamp(9px,0.75vw,10px)', marginTop:2 }}>{f.unit}</div>
                  ...
                  <div style={{ color:scoreColor, fontSize:'clamp(9px,0.75vw,10px)', marginTop:3, fontWeight:600 }}>Score: {score}/4</div>
                </div>
              )
            })}
          </div>
          ...
          <div style={{ background:'#081020', border:'1px solid #1e3050', borderRadius:10, padding:'clamp(12px,1.2vw,16px)', marginBottom:14 }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))', gap:10, fontSize:'clamp(10px,0.85vw,12px)', color:'#4a6080', fontFamily:'JetBrains Mono,monospace', lineHeight:1.8 }}>
              <div>
            ...
            <div style={{ marginTop:12, background:'rgba(96,165,250,0.06)', border:'1px solid rgba(96,165,250,0.15)', borderRadius:8, padding:'10px 12px' }}>
              <span style={{ color:'#60a5fa', fontWeight:600, fontSize:'clamp(11px,0.95vw,13px)' }}>Rule: </span>
              <span style={{ color:'#8099b8', fontSize:'clamp(11px,0.95vw,13px)' }}>Final state = maximum score across all vitals. One critical vital is enough to classify the patient as critical.</span>
            </div>
          </div>
              ...
              {Object.entries(vitalScores).map(([key, score]) => {
                ...
                return (
                  <div key={key} style={{ background:c.bg, border:`1px solid ${c.border}`, borderRadius:8, padding:'6px 12px', textAlign:'center' }}>
                    <div style={{ color:'#4a6080', fontSize:'clamp(9px,0.75vw,10px)', fontWeight:600, textTransform:'uppercase', marginBottom:2 }}>{labels[key]}</div>
                    <div style={{ color:c.color, fontSize:'clamp(11px,0.95vw,13px)', fontWeight:700 }}>Score {score}</div>
                  </div>
                )
              })}
            </div>
            ...
            {[
              ...
              <div key={s} style={{ display:'flex', gap:14, padding:'clamp(10px,1vw,14px)', borderRadius:10, background:STATE_COLORS[s].bg, border:`1px solid ${STATE_COLORS[s].border}`, alignItems:'flex-start' }}>
                ...
                <div>
                  ...
                  <div style={{ color:'#8099b8', fontSize:'clamp(11px,0.95vw,13px)', marginBottom:4, lineHeight:1.5 }}>{desc}</div>
                  <div style={{ color:'#4a6080', fontSize:'clamp(10px,0.85vw,12px)', fontFamily:'JetBrains Mono,monospace' }}>{vitals}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <Section title="Problem Statement" sub="What are we solving and why does it matter?" accent="#2563eb">
        <div className="card" style={{ fontSize:'clamp(13px,1.1vw,15px)', color:'#8099b8', lineHeight:1.8 }}>
              ...
              <ul style={{ listStyle:'none', display:'flex', flexDirection:'column', gap:7 }}>
                {p.items.map(item => (
                  <li key={item} style={{ display:'flex', gap:8, alignItems:'flex-start' }}>
                    <span style={{ color:p.color, marginTop:2, flexShrink:0, fontSize:10 }}>◆</span>
                    <span style={{ color:'#8099b8', fontSize:'clamp(11px,0.95vw,13px)', lineHeight:1.5 }}>{item}</span>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
          {[
            ...
            <div key={r.title} style={{ background:`rgba(${r.rgb},0.1)`, border:`1px solid rgba(${r.rgb},0.3)`, borderRadius:10, padding:'clamp(12px,1.2vw,16px)', display:'flex', gap:14 }}>
              ...
              <div>
                <p style={{ color:'#ffffff', fontSize:'clamp(13px,1.1vw,15px)', fontWeight:700, marginBottom:6 }}>{r.title}</p>
                <p style={{ color:'#8099b8', fontSize:'clamp(11px,0.95vw,13px)', lineHeight:1.7 }}>{r.body}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <Section title="AI Workflow" sub="How RL and LLM work together — each doing only what it's good at" accent="#60a5fa">
        <div className="card" style={{ marginBottom:0 }}>
          <div style={{ display:'flex', flexDirection:'column', gap:0 }}>
            {[
              { icon:'◉', label:'Dr. Roy speaks',       desc:'Natural language — vitals, observations, concerns',           color:'#67e8f9', rgb:'56,189,248' },
              ...
              { icon:'◉', label:'Dr. Roy reads',        desc:'Personalized clinical response with full reasoning',          color:'#4ade80', rgb:'52,211,153'   },
            ].map((step, i) => (
              <div key={i} style={{ display:'flex', gap:14, alignItems:'flex-start' }}>
                ...
                <div style={{ paddingTop:8, paddingBottom: i < 4 ? 0 : 0 }}>
                  ...
                  <p style={{ color:'#4a6080', fontSize:'clamp(11px,0.95vw,13px)' }}>{step.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="grid-2">
          <div className="card" style={{ borderLeft:'2px solid #a5b4fc' }}>
            <p style={{ color:'#a5b4fc', fontSize:'clamp(11px,0.95vw,13px)', fontWeight:700, marginBottom:8, textTransform:'uppercase', letterSpacing:'0.04em' }}>RL Agent — The Brain</p>
            <p style={{ color:'#8099b8', fontSize:'clamp(11px,0.95vw,13px)', lineHeight:1.7 }}>Decides WHAT to do. Never speaks. Never sees text. Only knows state numbers and Q-values. Outputs a single integer: 0, 1, 2, or 3.</p>
          </div>
          <div className="card" style={{ borderLeft:'2px solid #c084fc' }}>
            <p style={{ color:'#c084fc', fontSize:'clamp(11px,0.95vw,13px)', fontWeight:700, marginBottom:8, textTransform:'uppercase', letterSpacing:'0.04em' }}>LLM — The Voice</p>
            <p style={{ color:'#8099b8', fontSize:'clamp(11px,0.95vw,13px)', lineHeight:1.7 }}>Decides HOW to say it. Never makes medical decisions. Only translates between human language and structured data.</p>
          </div>
        </div>
      </Section>
      ...
      <Section title="Triage Agent — MDP Design" sub="Phase 1 · DQN · Manages a single patient" accent="#fb7185">
            ...
            {[
              ...
              <div key={a} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:14, padding:'clamp(10px,1vw,14px)', borderRadius:10, background:'#081020', border:'1px solid #1e3050', flexWrap:'wrap' }}>
                <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                  ...
                  <p style={{ color:'#8099b8', fontSize:'clamp(11px,0.95vw,13px)' }}>{desc}</p>
                </div>
                <div style={{ display:'flex', alignItems:'center', gap:5, flexShrink:0 }}>
                  <span style={{ color:'#4a6080', fontSize:'clamp(10px,0.85vw,11px)' }}>Legal in:</span>
                  {[0,1,2,3,4].map(s => (
                    <span key={s} style={{
                      width:26, height:26, borderRadius:6, display:'inline-flex', alignItems:'center', justifyContent:'center',
                      fontSize:'clamp(10px,0.85vw,11px)', fontWeight:700, fontFamily:'JetBrains Mono,monospace',
        ...
        <div className="card-glow">
          ...
          <p style={{ color:'#4a6080', fontSize:'clamp(11px,0.95vw,13px)', marginBottom:14 }}>Select a state to explore all legal actions and their outcome distributions</p>
          <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:18 }}>
            {[0,1,2,3,4].map(s => (
              <button key={s} onClick={() => setTriageState(s)} style={{
                padding:'6px 14px', borderRadius:20, fontSize:'clamp(11px,0.95vw,13px)', fontWeight:700,
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10, marginBottom:14 }}>
            {[
              ...
              <div key={r.case} style={{ textAlign:'center', background:'#081020', borderRadius:10, padding:'clamp(10px,1vw,14px)' }}>
                ...
                <div style={{ color:'#4a6080', fontSize:'clamp(10px,0.85vw,12px)', marginTop:3 }}>{r.formula}</div>
              </div>
            ))}
          </div>
          <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
            {[
              ...
              <div key={r.cond} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'clamp(8px,0.9vw,12px) clamp(12px,1.2vw,16px)', borderRadius:8, background:`rgba(${r.rgb},0.06)`, border:`1px solid rgba(${r.rgb},0.18)` }}>
                <span style={{ color:'#8099b8', fontSize:'clamp(11px,0.95vw,13px)' }}>{r.cond}</span>
      ...
      <Section title="Supervisor Agent — MDP Design" sub="Phase 2 · Q-Learning · Manages the entire ward" accent="#c084fc">
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:8, marginBottom:14 }}>
            {[
              ...
              <div key={w} style={{ display:'flex', alignItems:'center', gap:14, padding:'clamp(8px,0.9vw,12px)', borderRadius:8, background:WARD_COLORS[w].bg, border:`1px solid ${WARD_COLORS[w].border}` }}>
                <div style={{ width:38, height:38, borderRadius:8, background:WARD_COLORS[w].bg, border:`1px solid ${WARD_COLORS[w].border}`, display:'flex', alignItems:'center', justifyContent:'center', color:WARD_COLORS[w].color, fontSize:'clamp(11px,0.95vw,13px)', fontWeight:800, flexShrink:0, fontFamily:'JetBrains Mono,monospace' }}>W{w}</div>
                <div style={{ flex:1 }}>
                ...
                <code style={{ color:'#4a6080', fontSize:'clamp(10px,0.85vw,11px)', fontFamily:'JetBrains Mono,monospace', background:'#081020', padding:'3px 8px', borderRadius:5, flexShrink:0 }}>{rule}</code>
              </div>
            ))}
          </div>
          <div style={{ background:'#081020', borderRadius:8, padding:'clamp(10px,1vw,14px)', fontFamily:'JetBrains Mono,monospace', fontSize:'clamp(10px,0.85vw,12px)', color:'#4a6080', lineHeight:2 }}>
        ...
        <div className="card-glow">
          ...
          <p style={{ color:'#4a6080', fontSize:'clamp(11px,0.95vw,13px)', marginBottom:14 }}>Select a ward state to see supervisor action outcomes</p>
          <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:18 }}>
            {[0,1,2,3,4].map(w => (
        ...
        <div className="card">
            ...
            <table className="data-table">
              ...
              <tbody>
                {[
                  ...
                  <tr key={w}>
                    <td>
                      <span style={{ display:'inline-flex', alignItems:'center', padding:'3px 10px', borderRadius:20, fontSize:'clamp(10px,0.85vw,12px)', fontWeight:700, background:WARD_COLORS[w].bg, color:WARD_COLORS[w].color, border:`1px solid ${WARD_COLORS[w].border}` }}>
                        W{w} · {label}
                      </span>
                    </td>
                    {vals.map((v, j) => (
          ...
          <div style={{ marginTop:12, background:'rgba(74,222,128,0.06)', border:'1px solid rgba(74,222,128,0.18)', borderRadius:10, padding:'clamp(10px,1vw,14px)' }}>
            ...
            <p style={{ color:'#8099b8', fontSize:'clamp(11px,0.95vw,13px)', lineHeight:1.6 }}>The agent learned that bringing new resources outperforms reshuffling existing ones during a crisis. Clinically correct — and independently discovered through reinforcement learning with zero hardcoded rules.</p>
          </div>
        </div>
      </Section>
              ...
              <ul style={{ listStyle:'none', display:'flex', flexDirection:'column', gap:7 }}>
                {s.items.map(item => (
                  <li key={item} style={{ display:'flex', gap:8, alignItems:'center' }}>
                    ...
                    <span style={{ color:'#8099b8', fontSize:'clamp(11px,0.95vw,13px)' }}>{item}</span>
                  </li>
                ))}
              </ul>
            </div>
